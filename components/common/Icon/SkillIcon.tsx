...
import { SkillEnum, Tool } from "@/types/skill";
...
const SkillIcon = ({ icon: Icon, name, pointColor, active }: SkillIconProps) => {
  const notTransparent = [SkillEnum.TypeScript, SkillEnum.JavaScript];
  return <div className="flex flex-col items-center gap-y-2 w-[60px]">
    ...
    >
      <div
        className={cn("size-full absolute transition-opacity duration-200 ease-in-out", notTransparent.includes(name as SkillEnum) ? "opacity-100" : "opacity-20")}
